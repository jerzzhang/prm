...
router.post('/:user_id/contacts', function(req, res) {
	dbModel.postContact(req.params.user_id, req.query.firstName, req.query.lastName, req.query.phoneNumber, req.query.email, req.query.location, req.query.company, req.query.position, function(err, res) {
		res.send(error);
		res.end(res);
	});
}); 
